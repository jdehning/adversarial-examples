...
#data_Y = np.array([[0,1] for _ in range(len(train_dogs))] + [[1,0] for _ in range(len(train_cats))])
...
def run_catdog():
    ...
    model.fit(X_train, Y_train, batch_size=batch_size, epochs=nb_epoch, verbose=2, callbacks=[history])
...
model.save("keras_model_cat_dogs1")
